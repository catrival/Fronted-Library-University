...
} from "../../features/api/studentSlice";
...
export default function StudentsFormEdit() {
    ...
    try {
      ...
      if (!response) {
        Swal.fire({
          ...
          icon: "error",
          title:
            "El estudiante no pudo ser actualizado, por favor verifique los datos",
      ...
      Swal.fire({
        ...
        icon: "error",
        title:
          "El estudiante no pudo ser actualizado, por favor verifique los datoszzzz",
        showConfirmButton: false,
        timer: 1500,
      });
    }
  ...
  const {
    ...
    isError,
    error,
  } = useGetStudentByIdQuery(params.id);
  if (isLoading)
    return (
