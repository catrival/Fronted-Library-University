...
import { useDeleteLoanMutation, useGetLoansQuery } from '../../features/api/loanSlice';
import Swal from 'sweetalert2'
import Moment from 'moment';
import { useGetStudentByIdQuery, useGetStudentsQuery } from '../../features/api/studentSlice';
import { useGetBookByIdQuery, useGetBooksQuery } from '../../features/api/bookSlice';

export default function LoansList() {

    /** Obtiene el estado de una variable con Redux */
    // const loans = useSelector(state => state.loans)
    const { data: loans, isLoading, isError, error } = useGetLoansQuery();
        ...
        <div className="py-8 px-10 h-full">
            <table className="table-auto w-full">
                ...
                <tbody className="bg-gray-200 ">
                    {loans.map(loan => (
                        <tr key={loan.id}>
                            <td className="border-y-2 px-4 py-2">{Moment(loan.created).format('YYYY-MM-DD')}</td>
